...
namespace main_savitch_10
{
    ...
    ///
	    ...
	    {   // Move down the tree and add a new leaf:
          ...
          do
          {
          //entry is less than the data of the current node
            if (cursor->data( ) >= entry)
            {
              //no left node so entry must be inserted to the left of cursor
              if(cursor->left() == NULL)
                  done = true;
              else
                  //duplicates
          ...
          {
              //no right node so entry must be inserted to the right of cursor
              if(cursor->right() == NULL)
                  done = true;
              else
                  //keep searching the right subtree
                  cursor = cursor->right();
          }

        } while (!done);

        //create new node to insert entry
        binary_tree_node<Item>* bt = new binary_tree_node<Item>(entry);

        //determines if entry should go to the left or right of cursor
